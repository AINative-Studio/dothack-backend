...
class AINativeAuthClient:
    ...
    async def verify_token(self, token: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                ...
                extra={"event": "token_verification_error", "error_type": "connection_error", "error": str(e)},
            )
            raise AINativeConnectionError() from e
        except httpx.TimeoutException as e:
            logger.error(
                "Timeout during token verification",
                extra={"event": "token_verification_error", "error_type": "timeout", "error": str(e)},
            )
    ...
    async def _verify_token_with_retry(self, token: str) -> dict[str, Any]:
        ...
        if response.status_code == 200:
            ...
            logger.info(
                ...
                extra={"event": "token_verification_success", "user_id": user.get("id"), "email": user.get("email")},
            )
            return user
            ...
            if "expired" in error_detail.lower():
                logger.warning("Token expired", extra={"event": "token_verification_failed", "reason": "expired", "status_code": 401})
                raise TokenExpiredError()
            else:
                logger.warning("Invalid token", extra={"event": "token_verification_failed", "reason": "invalid", "status_code": 401})
                raise InvalidTokenError()

        else:
            logger.warning(
                "Token verification failed with unexpected status",
                extra={"event": "token_verification_failed", "status_code": response.status_code, "detail": response.text},
            )
    ...
    async def verify_api_key(self, api_key: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                ...
                extra={"event": "api_key_verification_error", "error_type": "connection_error", "error": str(e)},
            )
            raise AINativeConnectionError() from e
        except httpx.TimeoutException as e:
            logger.error(
                "Timeout during API key verification",
                extra={"event": "api_key_verification_error", "error_type": "timeout", "error": str(e)},
            )
            raise AINativeTimeoutError() from e
    ...
    async def _verify_api_key_with_retry(self, api_key: str) -> dict[str, Any]:
        ...
        logger.info(
            ...
            extra={"event": "api_key_verification_start", "key_preview": api_key[:10] + "..." if len(api_key) > 10 else api_key},
        )
        ...
        if response.status_code == 200:
            ...
            logger.info(
                ...
                extra={"event": "api_key_verification_success", "user_id": user.get("id"), "email": user.get("email")},
            )
            return user

        elif response.status_code == 401 or response.status_code == 403:
            logger.warning("Invalid API key", extra={"event": "api_key_verification_failed", "status_code": response.status_code})
            raise InvalidAPIKeyError()

        else:
            logger.warning(
                "API key verification failed with unexpected status",
                extra={"event": "api_key_verification_failed", "status_code": response.status_code, "detail": response.text},
            )
